...
// ── DETERMINISTIC COLOR PALETTE BY COURSE CODE ───────────────────────────────
const COURSE_COLORS = [
...
// ── CONVERT BACKEND GRADE RESPONSE TO MARKS_DATA SHAPE ───────────────────────
function buildMarksMeta(assessmentsByType) {
  // assessmentsByType: { "Quiz": [{title, totalMarks, obtainedMarks}], ... }
  const sections = {};
  Object.entries(assessmentsByType).forEach(([backendType, items]) => {
    const sectionName = TYPE_TO_SECTION[backendType] || backendType;
    const entries = items.map(item => ({
      label:  item.title       ?? backendType,
      marks:  item.obtainedMarks ?? 0,
      max:    item.totalMarks    ?? 0,
    }));
    const total = entries.reduce((s, e) => s + e.max, 0);
    // avg/classMax/classMin not available from backend — use 0 as safe fallback
...
export default function StudentMarks() {
  ...
  const [apiMarks,   setApiMarks]   = useState({});   // { courseCode: marksMeta }
  const [apiEnrolled, setApiEnrolled] = useState([]); // [{ code, name, credits }]
  const [loading,    setLoading]    = useState(true);

  // ── FETCH PROFILE + GRADES ────────────────────────────────────────────────
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        if (profileRes && profileRes.student) {
          ...
          setUserName(s.name      ?? "Student");
          setUserId(s.rollNumber  ?? s.studentId ?? "");
        } else if (profileRes && profileRes.name) {
          setUserName(profileRes.name);
          setUserId(profileRes.rollNumber ?? profileRes.studentId ?? "");
        }

        // Grades → build marksMeta map + enrolled list
        if (gradesRes && Array.isArray(gradesRes.grades)) {
          const marksMap = {};
          const enrolledList = [];

          gradesRes.grades.forEach(g => {
            const code = g.courseCode ?? g.course?.courseCode ?? "";
            if (!code) return;

            // Build marksMeta from assessments grouped by type
            const assessmentsByType = g.assessments ?? {};
            marksMap[code] = buildMarksMeta(assessmentsByType);

            // Build enrolled entry matching CourseContext shape
            enrolledList.push({
              id:      code,
              code,
              name:    g.courseName     ?? g.course?.name ?? code,
              credits: g.creditHours    ?? g.course?.creditHours ?? 3,
              price:   0,
              prof:    "",
              slot:    "",
              time:    "",
          ...
          setApiMarks(marksMap);
          setApiEnrolled(enrolledList);
        }
      } catch (err) {
        console.error("StudentMarks fetch error:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  function getMarksMeta(courseCode) {
    ...
    if (MARKS_DATA[courseCode]) return MARKS_DATA[courseCode];
    return null;
  }

  // ── Deselect course if it's no longer enrolled ────────────────────────────
  useEffect(() => {
    if (sc) {
      const found = resolvedEnrolled.some(e => e.code === sc.code);
      if (!found) { ssc(null); ses({}); }
    }
  }, [resolvedEnrolled, sc]);
  ...
  useEffect(() => {
    ...
    if (hpi) {
      ...
      if (wr.current)  { wr.current.style.opacity = 0; wr.current.style.display = "none"; }
      return;
    }
    ...
    for (let i = 0; i < 60; i++) {
      let rh = "60,140,255";
      if (Math.random() > 0.6) { rh = "255,255,255"; }
      pts.push({
    ...
    function drw() {
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        if (p.y < -30) { p.y = cvs.height + 20; p.x = Math.random() * cvs.width; }
    ...
    function aft() {
      ...
      setTimeout(() => { if (wr.current) { wr.current.style.display = "none"; } }, 3000);
    }
  ...
  useEffect(() => {
    ...
    sun.position.set(-6, 12, 8); scn.add(sun);
    let rl = new THREE.PointLight(0x0055ff, 3, 40);
    rl.position.set(0, 6, 0); scn.add(rl);
    let fl = new THREE.PointLight(0x88ccff, 1.5, 25);
    fl.position.set(-8, -2, 5); scn.add(fl);
    ...
    function mki(x, y, z, r, clr, opc = 0.22) {
      ...
      msh.position.set(x, y, z); scn.add(msh);
    ...
    mki(-7,2,-6,2.2,0x1a78ff,0.20); mki(7,-1,-7,2.8,0x0055dd,0.16);
    mki(-2,4,-9,3.4,0x2266ee,0.13); mki(4,3,-4,1.6,0x4499ff,0.24);
    ...
    mkr(-6,4,-7,2.0,0x4499ff); mkr(3,-1,-5,1.5,0x1166dd);
    ...
    let ptv = [];
    for (let i = 0; i < cnt; i++) {
      ptp[i*3]=(Math.random()-.5)*34; ptp[i*3+1]=(Math.random()-.5)*22; ptp[i*3+2]=(Math.random()-.5)*18-6;
      ptv.push({ x:(Math.random()-.5)*.010, y:(Math.random()-.5)*.008 });
      let pck = Math.random();
      if (pck < .4)      { ptc[i*3]=.1; ptc[i*3+1]=.4;  ptc[i*3+2]=1.0; }
      else if (pck < .7) { ptc[i*3]=.4; ptc[i*3+1]=.8;  ptc[i*3+2]=1.0; }
      else               { ptc[i*3]=.9; ptc[i*3+1]=.95; ptc[i*3+2]=1.0; }
    }
    ...
    flr.position.y = -5.5; flr.material.transparent = true; flr.material.opacity = 0.30;
    scn.add(flr);

    let nmx = 0, nmy = 0;
    function omv(e) { nmx=(e.clientX/W)*2-1; nmy=-(e.clientY/H)*2+1; }
    document.addEventListener("mousemove", omv);

    let t = 0, aid;
    function lp() {
      aid = requestAnimationFrame(lp); t = t + .012;
      for (let i = 0; i < objs.length; i++) {
        ...
        if (o.rz) { o.mesh.rotation.z = o.mesh.rotation.z + o.rz; }
      }
      ...
      cam.lookAt(0,0,0); ren.render(scn,cam);
    }
    lp();

    function orz() { W=window.innerWidth; H=window.innerHeight; ren.setSize(W,H); cam.aspect=W/H; cam.updateProjectionMatrix(); }
    window.addEventListener("resize", orz);
    return () => { cancelAnimationFrame(aid); document.removeEventListener("mousemove", omv); window.removeEventListener("resize", orz); };
  }, []);

  // ── DERIVED VALUES FOR SELECTED COURSE ───────────────────────────────────
  let mm  = sc ? getMarksMeta(sc.code) : null;
  let secs = sc ? (mm ? Object.keys(mm.sections) : []) : [];

  const courseColorData = sc ? getCourseColor(sc.code) : {};
  // Prefer hardcoded color from MARKS_DATA if available
  const hardcoded = sc ? MARKS_DATA[sc.code] : null;
  let cc  = hardcoded?.color  ?? courseColorData.color  ?? "#1a78ff";
  let bgc = hardcoded?.bg     ?? courseColorData.bg     ?? "rgba(26,120,255,.08)";
  let bor = hardcoded?.border ?? courseColorData.border ?? "rgba(26,120,255,.28)";

  let tcr = resolvedEnrolled.reduce((s, c) => s + (c.credits ?? 0), 0);

  // ── COURSE TILES ──────────────────────────────────────────────────────────
  let cbody = null;
  if (resolvedEnrolled.length === 0) {
    cbody = (
      <div className="mrk-card-body">
        <span className="mrk-placeholder">
          {loading ? "// loading courses..." : "// no courses enrolled — visit Registration"}
        </span>
      </div>
    );
  } else {
    let trk = [];
    for (let i = 0; i < resolvedEnrolled.length; i++) {
      let crs  = resolvedEnrolled[i];
      let mtd  = getMarksMeta(crs.code);
      let hcol = MARKS_DATA[crs.code];
      let apic = getCourseColor(crs.code);
      let clr  = hcol?.color  ?? apic.color;
      let bgi  = hcol?.bg     ?? apic.bg;
      let brd  = hcol?.border ?? apic.border;
      let actv = sc && sc.code === crs.code;
      let hlab = mtd ? Object.keys(mtd.sections).includes("Lab Work") : false;
      let tid  = crs.code.replace(/[^A-Z0-9]/gi, "").slice(0, 6).toUpperCase();

      let ccls = "course-tile" + (actv ? " course-tile--active" : "");
      let csty = actv ? { background: bgi, borderColor: brd } : {};
      ...
      trk.push(
        <div key={crs.code} className={ccls} style={csty} onClick={() => hcs(crs)}>
          ...
          <div className="course-tile-foot">
            ...
            {hlab && <span className="course-tile-lab" style={{ color: clr }}>LAB</span>}
            <span className="course-tile-arrow" style={{ color: clr }}>›</span>
          </div>
        </div>
      );
    }
    cbody = <div className="course-track">{trk}</div>;
  }
    ...
    for (let i = 0; i < secs.length; i++) {
      ...
      if (secdata) {
        for (let j = 0; j < secdata.entries.length; j++) {
          tot = tot + secdata.entries[j].marks;
        }
      }
    }
    bdg2 = (
  ...
  let rbod = null;
  if (!sc) {
    rbod = <div className="mrk-card-body"><span className="mrk-placeholder">{"// select a course to view marks"}</span></div>;
    ...
    for (let i = 0; i < secs.length; i++) {
      ...
      let s1 = dta.entries.reduce((s, e) => s + e.marks, 0);
      let mt1 = dta.entries.reduce((s, e) => s + e.max, 0);
      let pct1 = mt1 === 0 ? 0 : Math.min(Math.round((s1 / mt1) * 100), 100);
      let iso  = !!es[scn];
      let icn  = si[scn] ?? "◆";
      ...
      if (iso) {
        ...
        for (let j = 0; j < dta.entries.length; j++) {
          ...
          let epc = en.max === 0 ? 0 : (en.marks / en.max) * 100;
          trs.push(
            <tr key={en.label}>
                ...
                <tr>
                  <td colSpan={4}>
                    <div className="tbl-foot-row">
                      ...
                      <span className="tbl-foot-val">{dta.avg}</span>
                      <span className="tbl-foot-sep">·</span>
                      <span className="tbl-foot-label">High</span>
                      <span className="tbl-foot-val tbl-foot-max">{dta.classMax}</span>
                      <span className="tbl-foot-sep">·</span>
                      <span className="tbl-foot-label">Low</span>
                      <span className="tbl-foot-val tbl-foot-min">{dta.classMin}</span>
                    </div>
                  </td>
                </tr>
              </tfoot>
      ...
      msecs.push(
        <div className={"marks-sec" + (iso ? " marks-sec--open" : "")} key={scn}>
          <div className="marks-sec-hd" onClick={() => ts(scn)}>
            ...
            <div className="marks-sec-right">
              ...
              <span className={"marks-sec-chevron" + (iso ? " open" : "")}>›</span>
            </div>
          </div>
          {tbd}
        </div>
  ...
  let pbod = null;
  if (!sc) {
    pbod = <div className="mrk-card-body"><span className="mrk-placeholder">{"// select a course to view chart"}</span></div>;
  } else if (secs.length === 0) {
    pbod = (
      <div className="mrk-card-body" style={{ flexDirection:"column", gap:16 }}>
        <span style={{ fontSize:42 }}>📊</span>
    ...
    for (let i = 0; i < secs.length; i++) {
      ...
      let s2 = dta2.entries.reduce((s, e) => s + e.marks, 0);
      let cp = 88;
      let yp = dta2.classMax > 0 ? (s2 / dta2.classMax) * cp : 0;
      if (yp > cp) { yp = cp; }
      let ap = dta2.classMax > 0 ? (dta2.avg / dta2.classMax) * cp : 0;
      if (ap > cp) { ap = cp; }
      let mp = dta2.classMax > 0 ? (dta2.classMin / dta2.classMax) * cp : 0;
      if (mp > cp) { mp = cp; }
      let slb = scn2.replace(" Exam","").replace("Assignments","Assign.");

      let aib = yp > 0 ? Math.min((ap / yp) * 100, 100) : 0;
      let mib = yp > 0 ? Math.min((mp / yp) * 100, 100) : 0;

      let bgr = cc;
      if (yp > 0) {
        let p1 = mib - .5, p2 = mib + .5, p3 = aib - .5, p4 = aib + .5;
    ...
    pbod = (
      <div className="chart-wrap">
        ...
        <div className="chart-bars">{ccols}</div>
      </div>
    );
  }

  // ── TAB CONTENT ───────────────────────────────────────────────────────────
  let hb2  = sc ? "linear-gradient(180deg," + cc + "," + cc + "88)" : "linear-gradient(180deg,var(--blue),var(--blue2))";
  let hb3  = sc ? "linear-gradient(180deg," + cc + "," + cc + "55)" : "linear-gradient(180deg,#7c3aed,#a78bfa)";
  let sttl = sc ? sc.name : "Select a course above";
          ...
          <div id="mrk-scroll">
            <div className="mrk-card" id="mrk-card1">
                ...
                <div className="mrk-card-title-wrap">
                  ...
                  <div>
                    ...
                    <div className="mrk-card-sub">
                      Spring 2025 · {resolvedEnrolled.length} courses
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="course-track-foot">
                ...
                credit hours
                <span className="cfoot-dot" />
                {resolvedEnrolled.length} courses enrolled
              </div>
            </div>

            <div className="marks-tab-container">
              <button className={"marks-tab" + (at === 'raw'  ? " active" : "")} onClick={() => sat('raw')}>Raw Marks</button>
              <button className={"marks-tab" + (at === 'perf' ? " active" : "")} onClick={() => sat('perf')}>Performance Overview</button>
              <button className={"marks-tab" + (at === 'dist' ? " active" : "")} onClick={() => sat('dist')}>Grade Distribution</button>
            </div>

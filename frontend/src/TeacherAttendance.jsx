...
const tabContainerStyle = {
  display: "flex", gap: "8px", margin: "24px 32px 12px", flexWrap: "wrap",
};
function tabBtnStyle(active) {
  return {
    ...
    background: active ? "linear-gradient(135deg, #1a78ff, #0050cc)" : "rgba(18,78,170,.08)",
...
export default function TeacherAttendance() {
    ...
    Promise.all([
      ...
      .then(([studRes, attRes]) => {
        ...
        const normalized = rawStudents.map((s) => ({
          ...
          display: s.rollNumber ?? s.studentId ?? s.regNo ?? null,
        }));
  ...
  const sectionTime = typeof rawTime === "object" && rawTime !== null
    ? `${rawTime.day ?? ""} ${rawTime.startTime ?? ""}–${rawTime.endTime ?? ""}${rawTime.room ? ` · ${rawTime.room}` : ""}`.trim()
    : rawTime;
  ...
  return (
    <>
              ...
              <AnimatePresence>
                {unsaved && (
                  <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="ta-unsaved-badge">
                    <span className="ta-unsaved-dot" />UNSAVED
                  </motion.div>
                )}
              </AnimatePresence>
          ...
          <AnimatePresence>
            {error && (
              <motion.div
                ...
                style={{ margin: "16px 32px 0", padding: "12px 20px", borderRadius: "10px", background: "rgba(255,77,106,0.12)", border: "1px solid rgba(255,77,106,0.3)", color: "#ff4d6a", fontSize: "14px", fontWeight: 600, display: "flex", alignItems: "center", gap: "10px" }}
              >
                <span>⚠</span>{error}
                <button onClick={() => setError(null)} style={{ marginLeft: "auto", background: "none", border: "none", color: "#ff4d6a", cursor: "pointer", fontSize: "16px" }}>✕</button>
              </motion.div>
            )}
          </AnimatePresence>
            ...
            {loadingSections ? (
              <div style={{ display: "grid", gridTemplateColumns: "460px 1fr", gap: "32px", alignItems: "start" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                  ...
                  <div className="glass-card ta-card" style={{ padding: "24px" }}>
                    {[...Array(5)].map((_, i) => <Skeleton key={i} height={48} style={{ marginBottom: 12 }} />)}
                  </div>
                </div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
                style={{ display: "grid", gridTemplateColumns: "460px 1fr", gap: "32px", alignItems: "start", minHeight: "100%" }}
              >
                <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                  <div className="glass-card ta-card" style={{ padding: 0, opacity: 1, transform: "none" }}>
                    <div style={tabContainerStyle}>
                      {sections.map((sec) => {
                        ...
                        return (
                          <motion.button key={sid} style={tabBtnStyle(activeTab === sid)} onClick={() => setActiveTab(sid)} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}>
                            {label}
                          </motion.button>
                        );
                      })}
                      ...
                      <>
                        <div className="ta-roster">
                          {students.map((s) => {
                            ...
                            return (
                              <motion.div key={s.id} className={`ta-student-row${isSelected ? " selected" : ""}`} onClick={() => setSelected(s.id)} whileHover={{ backgroundColor: "rgba(26,100,255,0.05)" }}>
                                ...
                                <div className="ta-toggle-wrap">
                                  <div className="ta-toggle">
                                    <motion.button whileTap={{ scale: 0.9 }} className={`ta-toggle-btn${status === "P" ? " p-active" : ""}`} onClick={(e) => { e.stopPropagation(); toggle(s.id, "P"); }}>P</motion.button>
                                    <motion.button whileTap={{ scale: 0.9 }} className={`ta-toggle-btn${status === "A" ? " a-active" : ""}`} onClick={(e) => { e.stopPropagation(); toggle(s.id, "A"); }}>A</motion.button>
                                  </div>
                                </div>
                              </motion.div>
                            );
                        ...
                        <div className="ta-summary-footer">
                          <div className="ta-sum-stat"><div className="ta-sum-num green">{presentCount}</div><div className="ta-sum-lbl">Present</div></div>
                          <div className="ta-sum-divider" />
                          <div className="ta-sum-stat"><div className="ta-sum-num red">{absentCount}</div><div className="ta-sum-lbl">Absent</div></div>
                          <div className="ta-sum-divider" />
                          <div className="ta-sum-stat"><div className="ta-sum-num">{students.length}</div><div className="ta-sum-lbl">Total</div></div>
                          <motion.button className={`ta-save-btn${unsaved ? " active-save" : ""}`} onClick={save} disabled={!unsaved || saving} whileHover={{ scale: unsaved ? 1.05 : 1 }} whileTap={{ scale: unsaved ? 0.95 : 1 }} style={{ marginLeft: "auto" }}>
                            {saving ? "Saving…" : "✓ Save"}
                          </motion.button>
                        </div>
                      </>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
                  {loadingRoster ? (
                    ...
                    <AnimatePresence mode="wait">
                      <motion.div key={selStudent.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }} style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
                        <div className="glass-card ta-detail-card" style={{ opacity: 1, transform: "none" }}>
                          <div className="ta-detail-top">
                            <div>
                              ...
                              <div className="ta-detail-meta">{selStudent.display ?? `${String(selStudent.id).slice(0, 8)}…`} &nbsp;|&nbsp; {sectionLabel}</div>
                            </div>
                            <div className={`ta-big-pct ${selColor}`} style={{ color: C.color, textShadow: `0 4px 16px ${C.shadow}` }}>
                              <AnimatedCounter value={selPct} /><span>%</span>
                            </div>
                          </div>
                          <div className="ta-bar-track">
                            <motion.div className={`ta-bar-fill ${selColor}`} initial={{ width: 0 }} animate={{ width: `${selPct}%` }} transition={{ duration: 0.8, ease: "easeOut" }} />
                          ...
                          <div className="ta-stat-row">
                            <div className="ta-stat-pill green"><div className="ta-stat-num"><AnimatedCounter value={allEntries.filter((r) => r.status === "P").length} /></div><div className="ta-stat-lbl">Present</div></div>
                            <div className="ta-stat-pill red"><div className="ta-stat-num"><AnimatedCounter value={allEntries.filter((r) => r.status === "A").length} /></div><div className="ta-stat-lbl">Absent</div></div>
                            <div className="ta-stat-pill blue"><div className="ta-stat-num"><AnimatedCounter value={allEntries.length} /></div><div className="ta-stat-lbl">Classes Held</div></div>
                          </div>
                        </div>
                        ...
                        <div className="glass-card ta-log-card" style={{ opacity: 1, transform: "none" }}>
                          ...
                          <div className="ta-log-list">
                            <div className={`ta-log-row${attendance[selStudent.id] === "A" ? " absent" : ""}`}>
                              ...
                              <div className={`ta-log-status ${attendance[selStudent.id] === "P" ? "green" : "red"}`}>{attendance[selStudent.id] === "P" ? "Present" : "Absent"}</div>
                            </div>
                            {[...selHistory].reverse().map((r, i) => (
                              <div key={i} className={`ta-log-row${r.status === "A" ? " absent" : ""}`}>
                                ...
                                <div className={`ta-log-status ${r.status === "P" ? "green" : "red"}`}>{r.status === "P" ? "Present" : "Absent"}</div>
                              </div>
                            ))}
                          </div>
                        </div>
      ...
      <AnimatePresence>
        {showToast && (
          <motion.div className="ta-toast" initial={{ opacity: 0, y: 40, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 40, scale: 0.9 }} transition={{ duration: 0.3, type: "spring" }}>
